...
- **Symbol (DRC‑checked)**: ![Inverter Symbol DRC](schematics/inverter_symbol_drc.png)
- **Layout (TB view)**: ![Inverter Layout TB](schematics/inverter_layout_tb.png)

### Complete PFD Implementation
- **Schematic**: ![PFD Schematic](schematics/pfd_schematic.png)
- **Symbol (DRC‑checked)**: ![PFD Symbol DRC](schematics/pfd_symbol_drc.png)
- **Layout (TB view)**: ![PFD Layout TB](schematics/pfd_layout_tb.png)
...
### Inverter Building Block
- **Schematic**: *not provided* (add when available)
- **Symbol**: *not provided* (add when available)
- **Layout**: *not provided* (add when available)
...
### Inverter Transient Response
The inverter cell was simulated to verify correct switching behavior.

![Inverter Transient Response](plots/inverter_op_waveform.png)

### PFD Output Waveforms (Combined)
The combined waveform shows CLKREF (red), CLKVCO (green), UP (orange) and DOWN (yellow) signals across the full simulation window.
...
*Only the two screenshots currently available are embedded. Add additional figures (schematics, symbols, layouts) when they become available.*
